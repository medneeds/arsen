...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM patients WHERE id = v_patient_id AND is_vacant = TRUE) THEN
    RAISE NOTICE 'SKIP: UCC L03 já está vago — migração já aplicada.';
    RETURN;
  END IF;

  IF EXISTS (SELECT 1 FROM patient_encounters WHERE id = v_encounter_id AND status = 'closed') THEN
    RAISE NOTICE 'SKIP: encounter já está closed — migração já aplicada.';
    RETURN;
  END IF;

  SELECT archive_patient_bed_data(
    v_patient_id,
    'alta_hospitalar_liberacao_pendente_26mai2026'
  ...
  UPDATE patients SET
    ...
    highlighted_pendencies      = ARRAY[]::integer[],
    highlighted_diagnoses       = ARRAY[]::integer[],
    highlighted_medical_history = ARRAY[]::integer[],
    highlighted_conducts        = ARRAY[]::integer[],
    updated_at                  = NOW()
  WHERE id = v_patient_id AND is_vacant = FALSE;

  UPDATE patient_encounters SET
    status         = 'closed',
    discharge_date = '2026-05-26 13:17:26+00',
    updated_at     = NOW()
  WHERE id = v_encounter_id AND status = 'active';

  UPDATE patient_movements SET
    release_status   = 'released',
    released_at      = NOW(),
    released_by_name = 'Migration: liberação pendente 26/05/2026'
  WHERE id = v_movement_id AND release_status = 'pending_release';
  ...
  INSERT INTO patient_movements (
    ...
    'DIOGO COSTA DE ASSUNCAO', 'L03', 'ucc',
    'ALTA_HOSPITALAR',
    'PRONTUÁRIO PRESERVADO — LEITO LIBERADO',
    'Migration de resgate: alta sinalizada em 26/05/2026 com release_status=pending_release nunca confirmado. Admission_status remanesceu em admitido. Leito liberado via migration com archive prévio. Encounter 46d2a77a fechado.',
    v_state_id, v_hospital_unit_id,
    'released', NOW()
  );

  RAISE NOTICE 'SUCESSO: UCC L03 liberado, encounter fechado, histórico de Diogo preservado.';
