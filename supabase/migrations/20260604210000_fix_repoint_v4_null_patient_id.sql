...
-- ════════════════════════════════════════════════════════════════════
-- repoint_patient_history v4 — cobre evoluções com patient_id = null
-- ════════════════════════════════════════════════════════════════════
--
-- PROBLEMA CORRIGIDO:
--   Na v3, o bloco 3 (clinical_evolutions) tinha duas condições no WHERE:
--     1. patient_id = p_source_patient_id         ← cobre o caso normal
--     2. patient_registry_id = registry AND        ← cobre por encounter
--        v_source_encounter_id IS NOT NULL AND
--        encounter_id = v_source_encounter_id
--
--   Evoluções criadas por fluxo de pré-admissão têm patient_registry_id
--   preenchido mas patient_id = null. Nenhuma das duas condições as encontra:
--     1. patient_id = null ≠ p_source_patient_id  → não passa
--     2. v_source_encounter_id IS NOT NULL         → falso se encounter fechado
--        na Etapa 1 da transferência em dois estágios
--
-- MUDANÇA (cirúrgica — apenas uma linha nova no WHERE do bloco 3):
--   Adicionada terceira condição OR:
--     OR (patient_registry_id = v_registry_id AND patient_id IS NULL)
--
--   Captura exatamente o gap: registry correto + patient_id ausente.
--   Condições existentes permanecem sem alteração.
--
-- ROLLBACK:
--   Re-executar supabase/migrations/20260604200000_fix_repoint_propagate_critical_errors.sql
--
-- TABELAS AFETADAS: nenhuma estruturalmente
-- DADOS AFETADOS:   apenas evoluções com patient_id = null são atualizadas
--                   (nunca foram migradas antes — gap histórico)
-- ════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- 3) clinical_evolutions — CRÍTICO: sem EXCEPTION handler (v3)
  --    v4: adicionada terceira condição OR para patient_id = null
  ...
  -- 4) prescriptions — CRÍTICO: sem EXCEPTION handler (v3)
  ...
  -- 11) Tabelas secundárias — falhas silenciadas
  BEGIN
