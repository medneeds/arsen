...
begin
  ...
  g_r3 as (
    ...
    from base b
    where 'R3' = any(p_rules) and b.full_name is not null and b.birth_date is not null
  ...
  g_r4 as (
    select 'R4'::text as rule,
           ...
           array_agg(b.id order by b.created_at) as ids
    from base b
    where 'R4' = any(p_rules) and b.full_name is not null and b.birth_date is not null
    group by 1, 2 having count(*) > 1
  ),
  g_r5 as (
  ...
  g_r6 as (
    ...
    from base b1
    join base b2 on b2.id > b1.id and b2.birth_date = b1.birth_date and b2.birth_date is not null
  ...
  member_data as (
    select
      e.rule, e.key, e.ids_hash, e.ids, e.member_id, e.ord,
      jsonb_build_object(
        'id', b.id,
    ...
    from exploded e
    join base b on b.id = e.member_id
  ),
  grouped as (
