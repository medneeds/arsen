...
export function ProfileChooser({ userName, profiles, appRole, compact, onChosen }: Props) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      <div className="flex-1 flex items-start sm:items-center justify-center px-3 sm:px-4 py-6 sm:py-8">
        <motion.div
        ...
        >
          <div className="text-center mb-6 sm:mb-8">
            <div className="inline-flex items-center gap-2 text-xs font-medium text-muted-foreground tracking-[0.2em] mb-3">
              <span className="h-1.5 w-1.5 rounded-full bg-accent animate-pulse" />
              SELEÇÃO DE PERFIL
            </div>
            <h1 className="preserve-case text-xl sm:text-2xl md:text-3xl font-medium mb-2">
          ...
          <motion.button
            ...
            className="group w-full text-left relative bg-primary/5 border border-primary/30 rounded-lg p-4 sm:p-4 md:p-6 hover:border-primary/60 hover:shadow-md hover:shadow-md active:scale-[0.99] transition-all mb-3"
          >
            <span className="preserve-case absolute top-2.5 right-2.5 sm:top-3 sm:right-3 inline-flex items-center gap-1 text-xs sm:text-xs font-semibold tracking-[0.15em] sm:tracking-[0.18em] text-primary bg-primary/15 px-2 sm:px-2 py-1 sm:py-1 rounded-full">
              <Sparkles className="h-2.5 w-2.5" />
              PRINCIPAL
            </span>
            <div className="flex items-start gap-3 sm:gap-4">
              <div className="inline-flex items-center justify-center h-12 w-12 sm:h-14 sm:w-14 rounded-lg bg-primary/25 border border-primary/25 shrink-0">
                <PrimaryIcon className="h-5 w-5 sm:h-6 sm:w-6 text-primary" />
              </div>
              <div className="flex-1 min-w-0 pr-8 sm:pr-24">
                <p className="preserve-case font-medium text-sm sm:text-base md:text-lg text-foreground mb-1">
                  {primary.label}
                </p>
                <p className="preserve-case text-xs md:text-sm text-muted-foreground leading-relaxed line-clamp-3">
                  {primary.description}
                </p>
                <div className="mt-2 sm:mt-3 inline-flex items-center gap-2 text-xs font-medium text-primary">
          ...
          {rest.length > 0 && (
            <>
              <div className="flex items-center gap-2 mt-6 mb-3">
                ...
                <span className="preserve-case text-xs font-medium text-muted-foreground tracking-[0.2em]">
                ...
                {rest.map((p, idx) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className="group text-left relative bg-card border border-border/70 rounded-lg p-4 sm:p-4 hover:border-primary/40 hover:shadow-md hover:shadow-md active:scale-[0.99] transition-all"
                    >
                      <div className="flex items-start gap-3">
                        <div className="inline-flex items-center justify-center h-10 w-10 rounded-lg bg-primary/15 border border-primary/15 shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-primary opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
