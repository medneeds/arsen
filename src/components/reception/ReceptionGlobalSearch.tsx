      ...
      <CommandList className="max-h-[400px]">
        ...
        {!loading && query.trim().length < 2 && (
          <div className="py-8 text-center text-xs text-muted-foreground">
            ...
            <p className="mt-1 text-xs">Atalho: <kbd className="px-2 py-1 rounded-md border bg-muted font-mono text-xs">Ctrl + K</kbd></p>
          </div>
        )}
          ...
          <CommandGroup heading={`Atendimentos (${encounters.length})`}>
            {encounters.map((e) => (
              ...
              >
                ...
                <Badge
                  variant="outline"
                  className="text-xs h-4 shrink-0 capitalize"
                >
                  {e.status}
                </Badge>
              </CommandItem>
