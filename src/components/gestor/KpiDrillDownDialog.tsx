    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        ...
        <ScrollArea className="flex-1 -mx-6 px-6">
          {rows.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8 preserve-case">{emptyLabel}</p>
          ) : (
              ...
              {rows.map((row) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    {row.secondary && (
                      <p className="text-xs text-muted-foreground mt-1 preserve-case">{row.secondary}</p>
                    )}
                    {row.tertiary && (
                      <p className="text-xs text-muted-foreground/70 mt-1 preserve-case line-clamp-2">{row.tertiary}</p>
                    )}
                  </div>
                  {row.badge && (
                    <Badge variant={row.badge.variant || "outline"} className="text-xs shrink-0">
                      {row.badge.label}
                    </Badge>
                  )}
                </li>
