...
description: Cockpit clínico (sidebar) sincroniza em tempo real com patients, admission_histories, prescriptions, exam_requests, culture_results, patient_movements, clinical_evolutions, vital_signs, round_sessions e bed_allocation_requests via Supabase Realtime
type: feature
---
...
- `useLatestRoundSession(patientId)` ouve `round_sessions`, `round_responses` e `round_section_goals` — card com data do round + contagem de respostas/metas e timestamp da última atualização.
- `usePatientNirRequest(patientId)` ouve `bed_allocation_requests` — card com status (Pendente/Em discussão/Aprovada/Rejeitada), setor/leito solicitado, justificativa de rejeição; toast em mudança de status (success/info/error).
...
- Tabelas com REPLICA IDENTITY FULL na publication `supabase_realtime`: patients, admission_histories, clinical_evolutions, prescriptions, exam_requests, culture_results, patient_movements, vital_signs, round_sessions, round_responses, round_section_goals, bed_allocation_requests.
